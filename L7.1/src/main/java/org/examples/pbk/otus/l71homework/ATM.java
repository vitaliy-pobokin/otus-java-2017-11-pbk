...
public class ATM {
    ...
    public AtmState saveState() {
        return new AtmState(cells.values());
    }

    public void restoreState(AtmState state) {
        cells = new HashMap<>();
        for(AtmState.BanknoteCellState cellState : state.getState()) {
            BanknoteCell cell = new BanknoteCell(cellState.type, cellState.banknoteQuantity, cellState.cellSize);
            cells.put(cell.getBanknoteType(), cell);
        }
        System.out.println("ATM state was restored.");
    }
